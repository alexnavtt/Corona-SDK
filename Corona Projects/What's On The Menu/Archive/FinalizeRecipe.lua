...
local function createIngredientPanel(foodname)
	...
	local integer_text = display.newText({text = "",
										  x = integer_rect.x,
										  y = integer_rect.y,
	...
	local numerator_text = display.newText({text = "",
											x = numerator_rect.x,
											y = numerator_rect.y, 
	...
	local denominator_text = display.newText({text = "2",
	...
	local unit_text = display.newText({text = "cup",
									   x = unit_rect.x,
									   y = unit_rect.y,
...
function scene:show( event )
	...
	if ( phase == "will" ) then
		...
		local y_level_delta = 2*y_level

		for index, value in pairs(cookbook.newRecipeIngredientList) do
			local new_ingredient_panel = createIngredientPanel(value.name)
